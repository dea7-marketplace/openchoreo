...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	staleResources := r.findStaleResources(liveResources, desiredResources, release.Status.Resources)
...
func (r *Reconciler) findStaleResources(
	liveResources, desiredResources []*unstructured.Unstructured,
	previouslyApplied []openchoreov1alpha1.RenderedManifestStatus,
) []*unstructured.Unstructured {
	desiredIdentities := make(map[resourceIdentity]struct{}, len(desiredResources))
	desiredResourceIDs := make(map[string]struct{}, len(desiredResources))
	for _, obj := range desiredResources {
		desiredIdentities[identityForObject(obj)] = struct{}{}
		resourceID := obj.GetLabels()[labels.LabelKeyRenderedReleaseResourceID]
		if resourceID != "" {
			desiredResourceIDs[resourceID] = struct{}{}
		}
	}

	previousIdentities := make(map[resourceIdentity]struct{}, len(previouslyApplied))
	for _, status := range previouslyApplied {
		previousIdentities[identityForStatus(status)] = struct{}{}
	}
	...
	for _, liveObj := range liveResources {
		...
		identity := identityForObject(liveObj)
		if _, desired := desiredIdentities[identity]; desired {
			continue
		}

		// A previously inventoried rendered root whose exact identity disappeared
		// is stale even if its logical resource ID is reused by a replacement.
		if _, previouslyRendered := previousIdentities[identity]; previouslyRendered {
			staleResources = append(staleResources, liveObj)
			continue
		}

		// Keep children generated by another controller. Deleting their rendered
		// root cascades through the owner reference.
		if hasControllerOwner(liveObj) {
			continue
		}

		// Retain un-inventoried objects while their logical entry exists. This is
		// the compatibility path for generated objects without owner references.
		if _, desiredID := desiredResourceIDs[liveResourceID]; !desiredID {
			staleResources = append(staleResources, liveObj)
		}
	}
